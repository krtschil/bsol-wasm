...
**Important caveat, verified in this sandbox:** this environment has
exactly **one CPU core** (`nproc` → 1). Running the same 40 boards with
1 / 2 / 4 workers here measured **11.7s / 12.8s / 13.6s** respectively -
i.e. *slower* with more workers, because there is no physical
parallelism available; each extra worker only adds module-load and
context-switch overhead on top of the same single core. This confirms
the aggregation logic itself is correct (board counts, solved/error
counts all matched exactly across worker counts) - the tool works as
designed, but a single-core sandbox cannot demonstrate the speedup it's
meant to measure. On a real multi-core development or CI machine,
expect wall-clock time to drop close to linearly with worker count, up
to the number of physical cores available, then flatten out.

**A real distribution bug found and fixed while testing this on an
actual multi-core machine:** the first version of the multi-worker
split divided the file by raw **line index** modulo worker count
(`lineIndex % numWorkers`). That looks reasonable for a file with
exactly one line per board, but silently breaks on realistic `.pbn`
files with several header lines per record (`[Event]`, `[Site]`,
`[Board]`, ..., `[Deal]`, ...): if the number of lines per record
shares a common factor with the worker count (e.g. 8 lines/record with
8 workers), every single `[Deal]` line lands on the *same* modulo
bucket - one worker receives all the boards, the other N-1 receive
none. Symptom observed: `workers=8` correctly requested and logged, yet
only one CPU core busy and no speedup, because 7 of the 8 workers
finished instantly with nothing to do. Fixed by filtering the input
down to actual deal-bearing lines *first*, then round-robining across
*those* - verified to produce an exactly even split (5/5/5/5/5/5/5/5
boards per worker for a 40-board, 8-header-lines-per-record test file
that reproduced the bug 40/0/0/0/0/0/0/0 before the fix).

**A third distribution bug, found by the same person, in real PBN files
with both `[Dealer ...]` and `[Deal ...]` tags:** the deal-line filter
used a plain prefix match, `/^\[Deal/`, to decide which lines count as
"a board" for splitting purposes. That prefix also matches the
*different* `[Dealer "N"]` tag (who deals first) - since "Dealer"
starts with the same four letters as "Deal" - because it's a prefix
match, not a match against the whole tag name. Standard PBN files often
have exactly one `[Dealer ...]` line immediately paired with one
`[Deal ...]` line per board record, alternating 1:1 through the file.
With exactly 2 workers, that 1:1 alternation combined with
`lineIndex % 2` round-robin sends *every* `[Dealer ...]` line to worker
0 and *every* `[Deal ...]` line to worker 1 - worker 0 receives zero
real boards (its "chunk" is nothing but harmless-looking `[Dealer "N"]`
lines that the underlying wasm code correctly rejects as not being
real deals, but only after they've already displaced real work from
that worker), while worker 1 does 100% of the actual solving alone.
Symptom observed: `--workers=2` requested, but the total time barely
differs from `--workers=1` since only one worker was ever doing real
work; `--workers=3` looked "faster" because a 1:1 alternating pattern
does *not* divide evenly into 3 buckets, so some real `[Deal]` lines
inevitably ended up spread across more than one worker by chance.
Fixed by requiring whitespace immediately after `Deal`
(`/^\[Deal\s/` in `bench_pbn_cli.js`, `line.rfind("[Deal ", 0) == 0`
with an explicit trailing space in the wasm-side parser) - this matches
`[Deal "N:..."]` but not `[Dealer "N"]`, since PBN tags are always
`[TagName "value"]` with a space directly after the tag name. Verified
with a 20-board file alternating `[Dealer ...]`/`[Deal ...]` 1:1: before
the fix, `--workers=2` produced a 0/20 split; after the fix, an exact
10/10 split.

**A second bug, found by the same person: `avgMs`/`medianMs` were wrong
in multi-worker mode, and got *more* wrong the more workers were used.**
The original formula computed `avgMs = wallMs / totalBoardsAcrossAllWorkers`
- dividing the parallel wall-clock time by a board count that grows
with worker count conflates a *throughput* measure with a *latency*
measure. The more workers, the smaller (and less meaningful) the
reported average became, even with zero real speedup - purely an
artifact of the formula, not of anything the hardware was actually
doing. The median had a related, milder issue: it was approximated as
the median of each worker's own median, not a true median over every
individual board.
