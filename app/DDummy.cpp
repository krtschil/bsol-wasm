/**********************************************************************************
   ...
   -- (KK)
   - Small changes due to warnings in the build process (confirmed by Claude)
	- Deactivated the function "int playCard" as it is not used anywhere
	- Conversion from string literal to 'char *' requires "const" to the definition
	- Casting to (int) line 815
***********************************************************************************/
#include "dll.h"
#include "timer.h"
#include <sys/un.h>
...
/* Unused function
 * Deactivated: verified with Claude
 *
int playCard(char* pbn,char trump,char leader,char* currentTrickCards,char* response)
{
	const char* suitLetters  = "SHDCN";
	const char* cardLetters = "23456789TJQKA";
...
int playAccuracy(char* pbn,char trump,char leader, char* cards,char* response)
{
	...
	timer.check();
	/* A cast to (int) needed to avoid a compiler warning: format specifies type 'int' but the argument has type 'unsigned long'
	 *
	 */
	snprintf(response,500,"{\"declErr\":%d,\"defErr\":%d,\"totalCardsPlayed\":%d,\"deltaElapsed\":%f",declErr,defErr,(int)(strlen(cards)/2),timer.dblElapsed());
